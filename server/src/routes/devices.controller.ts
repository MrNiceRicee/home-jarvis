...
export const devicesController = new Elysia({ prefix: '/api/devices' })
    ...
    Promise.all(
      enabled.map(async (integration) => {
        try {
          ...
          const adapter = createAdapter(integration.brand, config)
          await adapter.discover()
        } catch {
          // adapter not yet implemented or discovery failed
        }
      })
    ).catch(console.error)
  ...
  .patch('/:id/state', async ({ db, params, body }) => {
    ...
    try {
      const adapter = createAdapter(integration.brand, config)
      await adapter.setState(device.externalId, body)
      ...
      eventBus.publish({ type: 'device:update', deviceId: params.id, brand: device.brand, state: newState, timestamp: now })
  ...
  .patch('/:id/homekit', async ({ db, params, body }) => {
    const device = db.select().from(devices).where(eq(devices.id, params.id)).get()
    if (!device) return new Response(JSON.stringify({ error: 'Device not found' }), { status: 404 })
    ...
    eventBus.publish({ type: 'device:update', deviceId: params.id, brand: device.brand, timestamp: now })
