...
export function createAdapter(brand: string, config: Record<string, string>): DeviceAdapter {
  switch (brand) {
    case 'hue':
      return new HueAdapter(config)
    // Phase 3–4 adapters are stubs until implemented
    default:
      throw new Error(`Adapter not yet implemented for brand: ${brand}`)
  }
}
