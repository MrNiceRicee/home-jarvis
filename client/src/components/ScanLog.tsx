		...
		<div>
			...
			>
				...
				>
					...
					>
						{isEmpty && <p className="text-display-text/50 py-2">awaiting scan...</p>}
						{entries.map((entry) => {
								...
								return (
									<div
										...
										className={cn(
											...
											allErrored ? 'text-red-400' : 'text-emerald-400',
										)}
									>
										<span>
											<ScrambleText value={entry.text} range={[0x2800, 0x28ff]} />
										</span>
										{onRescan && <TerminalButton label="RESCAN" onPress={onRescan} />}
									</div>
								)
							}
							...
							return (
								...
								>
									...
									<span className="tabular-nums text-right">
										{entry.status === 'found' && (
											<ScrambleText value={`${entry.count} found`} range={[0x2800, 0x28ff]} />
										)}
						...
						{scanning && (
							<div className="grid grid-cols-[1fr_auto] gap-4 py-0.5 text-display-text/60">
								<span>
									scanning devices<span className="scan-cursor ml-0.5">{'\u2588'}</span>
								</span>
								{onRescan && <TerminalButton label="RESCAN" onPress={onRescan} isDisabled />}
							</div>
						)}
					</div>
				</ReadoutDisplay>
