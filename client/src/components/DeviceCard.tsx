...
export function DeviceCard({ device, onHomekitToggle, onStateChange }: DeviceCardProps) {
  ...
  return (
    <div className={`bg-white rounded-xl border transition-all ${device.online ? 'border-gray-200 shadow-sm' : 'border-gray-100 opacity-60'}`}>
      <div className="p-4">
        {/* Header */}
        <div className="flex items-start justify-between gap-2 mb-3">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs text-gray-400 truncate">{BRAND_LABEL[device.brand] ?? device.brand}</p>
            </div>
          </div>
        ...
        {(device.type === 'light' || device.type === 'switch') && device.online && (
          ...
          >
            {toggling ? 'Updating…' : state.on ? 'Turn Off' : 'Turn On'}
          </Button>
        )}
      </div>
      ...
      <div className="px-4 py-2.5 border-t border-gray-100 flex items-center justify-between">
        ...
        {isAqara ? (
          <TooltipTrigger delay={200}>
            <Button className="text-xs text-gray-300 cursor-default focus:outline-none">Native ✓</Button>
          ...
          >
            ...
            >
              <div className={`w-4 h-4 bg-white rounded-full shadow-sm m-0.5 transition-transform
...
function OnlineBadge({ online }: { online: boolean }) {
  return (
    <span className={`flex items-center gap-1 text-xs font-medium shrink-0 px-1.5 py-0.5 rounded-full
  ...
  if (type === 'light' || type === 'switch') {
    return (
      <div className="flex items-center gap-2 text-xs text-gray-500">
        ...
        {state.brightness !== undefined && state.on && (
