import {
  Button, Form, Input, Label, TextField, FieldError,
  Dialog, DialogTrigger, Modal, ModalOverlay, Heading,
} from 'react-aria-components'
import { useState } from 'react'
import type { IntegrationMeta, CredentialField } from '../types'
...
export function IntegrationCard({ meta, isConfigured, onSubmit, onRemove }: IntegrationFormProps) {
  return (
    <div className={`bg-white rounded-xl border transition-all ${isConfigured ? 'border-emerald-200 shadow-sm' : 'border-gray-200'}`}>
              ...
              <ModalOverlay className="fixed inset-0 bg-black/30 z-50 flex items-center justify-center entering:animate-in entering:fade-in exiting:animate-out exiting:fade-out">
                <Modal className="bg-white rounded-2xl shadow-xl w-full max-w-sm mx-4 entering:animate-in entering:zoom-in-95 exiting:animate-out exiting:zoom-out-95">
                  <Dialog className="p-6 outline-none">
                    {({ close }) => (
                      <>
                        <Heading slot="title" className="text-base font-semibold text-gray-900 mb-2">
                          Remove {meta.displayName}?
                        </Heading>
                        <p className="text-sm text-gray-500 mb-5">
                          All devices from this integration will be removed from the portal. HomeKit accessories will be unexposed.
                        </p>
                        <div className="flex gap-2 justify-end">
                          <Button onPress={close} className="px-3 py-1.5 text-sm rounded-lg bg-gray-100 text-gray-700 hover:bg-gray-200 cursor-default pressed:bg-gray-300 focus:outline-none focus-visible:ring-2 focus-visible:ring-gray-400">
                            Cancel
                          </Button>
                          <Button
                            onPress={async () => { await onRemove(meta.brand); close() }}
                ...
                <Dialog className="p-6 outline-none">
                  {({ close }) => (
                    <IntegrationFormInner
                      meta={meta}
                      onSubmit={async (config) => { await onSubmit(meta.brand, config); close() }}
                      onCancel={close}
                    />
                  )}
                </Dialog>
...
function IntegrationFormInner({ meta, onSubmit, onCancel }: InnerProps) {
  ...
  async function handleLinkHueBridge() {
    if (!values.bridgeIp) { setHueLinkError('Enter the bridge IP first'); return }
    setLinkingHue(true)
    setHueLinkError(null)
    try {
      const res = await fetch('/api/integrations/hue/link', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ bridgeIp: values.bridgeIp }),
      })
  ...
  if (meta.oauthFlow) {
    return (
      <div>
        <Heading slot="title" className="text-base font-semibold text-gray-900 mb-1">{meta.displayName}</Heading>
        <p className="text-sm text-gray-500 mb-5">Uses OAuth 2.0 — you'll be redirected to LG to authorize.</p>
        <div className="flex gap-2 justify-end">
          <Button onPress={onCancel} className="px-3 py-1.5 text-sm rounded-lg bg-gray-100 text-gray-700 hover:bg-gray-200 cursor-default pressed:bg-gray-300 focus:outline-none focus-visible:ring-2 focus-visible:ring-gray-400">
            Cancel
          </Button>
          <Button
    ...
    <Form onSubmit={handleSubmit}>
      ...
      {meta.brand === 'hue' && (
        <div className="mb-4">
          ...
          {values.apiKey && <p className="text-xs text-emerald-600 mt-1">✓ Bridge linked successfully</p>}
        </div>
      )}
...
function CredentialFieldInput({ field, value, onChange }: { field: CredentialField; value: string; onChange: (v: string) => void }) {
  return (
